...
# Step 2: Install dependencies with a compatible set of pinned versions
echo "Installing dependencies with pinned versions..."
pip install --upgrade pip

# This block contains a set of LangChain packages known to be compatible.
...
# Re-run torch install with specific index just in case
...
# Ensure log directory exists before starting Ollama
mkdir -p ~/chatbot-k8s-v2/logs
...
# Step 8: Find the absolute paths for kubectl and kubeconfig
echo "Finding and exporting Kubernetes configuration paths..."

# Find the absolute path to the kubectl executable
KUBECTL_PATH=$(which kubectl)
if [ -z "$KUBECTL_PATH" ]; then
    echo "FATAL: 'kubectl' command not found in your PATH. Please install it."
    exit 1
fi
echo "Found kubectl at: $KUBECTL_PATH"
export CHATBOT_KUBECTL_PATH="$KUBECTL_PATH"

# Define and export the absolute path to your kubeconfig file
KUBECONFIG_PATH="$HOME/.kube/config"
if [ ! -f "$KUBECONFIG_PATH" ]; then
    echo "FATAL: Kubeconfig file not found at: $KUBECONFIG_PATH"
    exit 1
fi
echo "Using kubeconfig at: $KUBECONFIG_PATH"
export CHATBOT_KUBECONFIG_PATH="$KUBECONFIG_PATH"
...
cd app
# The environment variables are now correctly set for the Python process
...
# Wait for Flask to exit, and then clean up Ollama
wait $FLASK_PID
echo "Flask server stopped. Shutting down Ollama..."
kill $OLLAMA_PID 2>/dev/null || true
